...
@synthesize bookmarkNew = _bookmarkNew;
...
- (void)saveAction {
    ...
    for (BookmarkObject * bookmark in _bookmarksArray) {
        if ([bookmark.url.absoluteString isEqual:_bookmarkNew.url.absoluteString]) {
            saveURL = NO;
            break;
        }
    }
    ...
    if (saveURL) {
        [_bookmarksArray addObject:_bookmarkNew];
        [self.tableView reloadData];
    }
            ...
            switch (indexPath.row) {
                case 0:
                {
                    ...
                    if (cell == nil) {
                        ...
                        _nameTextField.text = _bookmarkNew.name;
                ...
                case 1:
                {
                    ...
                    cell.textLabel.text = [_bookmarkNew.url absoluteString];
                    break;
                }
                default:
                {
...
- (void)setBookmark:(NSString *)aName url:(NSURL *)aURL {
    _bookmarkNew = [[BookmarkObject alloc] initWithName:aName andURL:aURL];
}
